        ...
        if (codon_file.is_open()) {
            ...
            for (std::string line; getline(codon_file, line);){
                ...
                if(line_split.size() != 3){
                    std::cout << "Wrong format of codon frequency file!" << std::endl;
                    exit(1);
                }
            ...
            if (codon_table_.size() != 64){
                std::cout << "Codon frequency file needs to contain 64 codons!" << std::endl;
                exit(1);
            }

        } else {
            std::cout << "The input codon frequency file does not exist!" << std::endl;
            exit(1);
        }
    }
